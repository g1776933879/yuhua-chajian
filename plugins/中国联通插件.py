...
# [param: {"required":false,"key":"yuhua_zglt.yuhua_zglt_ddname","bool":false,"placeholder":"Host丨app_key丨app_secret","name":"对接容器(呆呆面板)","desc":"留空则使用青龙面板。各参数之间用中文符丨分割，例如: http://127.0.0.1:5700丨abcdef-ghijk丨abcdefghijklmnopqrs_tuvw"}]
...
yuhua_zglt_ddname = middleware.bucketGet("yuhua_zglt", "yuhua_zglt_ddname") or ""
PANEL_KIND = "ql"   # ql=青龙, dd=呆呆
PANEL_BASE = ""     # 青龙 {host}/open, 呆呆 {host}/api

# 对接青龙
def seekql():
    global PANEL_KIND, PANEL_BASE
    try:
        # ===== 呆呆面板（填了呆呆配置就优先走呆呆）=====
        if len(yuhua_zglt_ddname) > 0:
            ddlist = yuhua_zglt_ddname.split('丨')
            if len(ddlist) != 3:
                sender.reply("""
=====格式错误=====
❌ 呆呆配置格式错误
...
def DDtoken(host, app_key, app_secret):  # 获取呆呆面板token
    try:
...
def _resp_ok(rj):
    """青龙用 code==200 判成功，呆呆用 success!=false"""
    if not isinstance(rj, dict):
        return False
    if 'code' in rj:
        return rj.get('code') == 200
