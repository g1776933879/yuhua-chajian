...
# -*- coding: utf-8 -*-
"""
♻️回收猿旧衣服回收【羽化插件/青龙面板双兼容版】
支持：
  1. 羽化云：直接导入为插件，使用羽化表单配置
  2. 青龙面板：直接运行，读取系统环境变量
环境变量/羽化配置项：
YYB_SERVER      YYB服务列表 格式：地址@账号ID，多行/逗号分隔
YYB_API_KEY     YYB服务密钥
PLUSPLUS_TOKEN  PushPlus推送token(选填)
PROXY_API       品赞代理API(选填)
PROXY_TYPE      代理类型 http/socks5 默认http
HSY_CHANNEL_ID  渠道ID 默认wx1008
HSY_WITHDRAW_MIN 最低提现金额 默认1

依赖：pip install requests requests[socks]
缓存文件：hsycookie.json（脚本同目录）
"""
import hashlib
import json
import os
...
# ===================== 羽化兼容层 start =====================
IS_YUHUA = bool(os.getenv("__YS_PLUGIN__", ""))
YS_PLUGIN_CONFIG = {}

if IS_YUHUA:
    try:
        # 羽化插件读取插件配置json
        cfg_raw = os.getenv("YS_PLUGIN_CONFIG", "{}")
        YS_PLUGIN_CONFIG = json.loads(cfg_raw)
    except Exception:
        YS_PLUGIN_CONFIG = {}

def get_env(key: str, default: str = "") -> str:
    """双模式读取配置：羽化优先读插件配置，否则读系统环境变量（青龙）"""
    if IS_YUHUA:
        val = YS_PLUGIN_CONFIG.get(key, "")
        if val is not None and str(val).strip() != "":
            return str(val).strip()
    return os.getenv(key, default).strip()
# ===================== 羽化兼容层 end =====================

# ------------ 这里保留原有 yyb_account_guard 导入 ------------
try:
    from yyb_account_guard import filter_accounts, update_from_result
except ImportError:
    print("⚠️警告：缺少 yyb_account_guard 模块，账号过滤功能将跳过")
...
# 读取配置（经过双兼容get_env）
_SERVER_ENV = get_env("YYB_SERVER", "").strip()
SERVERS = [item.strip() for item in _SERVER_ENV.replace(",", "\n").splitlines() if item.strip()]
SERVERS = filter_accounts(SERVERS, app_id=APPID, log=print)

YYB_API_KEY = get_env("YYB_API_KEY", "")
CHANNEL_ID = get_env("HSY_CHANNEL_ID", "wx1008")
WITHDRAW_MIN = float(get_env("HSY_WITHDRAW_MIN", "1"))
PLUSPLUS_TOKEN = get_env("PLUSPLUS_TOKEN", "")
PROXY_API = get_env("PROXY_API", "")
PROXY_TYPE = get_env("PROXY_TYPE", "http").lower()
...
def log_title() -> None:
    print()
    print("╔" + "═" * 50 + "╗")
    print("║ ♻️ 回收猿旧衣服回收双兼容版                ║")
    print(f"║ 🕒 启动时间: {now_text():<32}║")
    print(f"║ 🔢 账号数量: {len(SERVERS):<34}║")
    print(f"║ 🧩 运行模式: {'羽化插件' if IS_YUHUA else '青龙面板'}<28║")
    print("╚" + "═" * 50 + "╝")
